...
Vid kommande AWS-deployment ska load balancern ha HTTPS på port 443 med ett giltigt certifikat; eventuell publik port 80 ska endast omdirigera till HTTPS. För denna portfolio/demo placeras Fargate-tasks i publika subnet med publik IP för utgående HTTPS, men backendens port 8080 ska endast tillåta trafik från load balancerns security group. Databasen ska ligga i privata subnet med `publicly_accessible = false` och endast tillåta anslutningar från backendens security group på 5432. Applikationens interna HTTP-anslutning och HTTP-health check förutsätter denna nätverksgräns; backendens API ska inte vara direkt åtkomligt från internet.
...
## AWS-nätverk med Terraform

[infra/terraform](infra/terraform/README.md) innehåller ett VPC i `eu-north-1`, publika och privata subnet över två AZ, routing och security groups för kedjan HTTPS → ALB → backend:8080 → PostgreSQL:5432. Dessutom ingår privat PostgreSQL 17 RDS på `db.t4g.micro` med krypterad lagring, tvingad TLS och RDS-hanterat lösenord i Secrets Manager. Databasen kör Single-AZ; privata subnet har ingen internetroute. Ingen applikation, ALB eller ECS skapas ännu. Demo-databasen raderas utan slutlig snapshot vid destroy; se guidens instruktioner för backup.

Se [Terraform-guiden](infra/terraform/README.md) för kortlivad AWS/SSO-autentisering, variabler, `init`, `plan`, `apply`, `destroy`, state-hantering och verifiering. Framtida Fargate-tasks använder publika subnet med `assign_public_ip = true` och utgående HTTPS för att slippa NAT Gateways och VPC endpoints. Inkommande backendtrafik tillåts fortfarande endast från ALB, och RDS förblir privat. Befintlig CI och ECR-publicering ändras inte.
